...
for TARGET in $TARGETS; do
  ...
  if ([ $XGOOS == "." ] || [ $XGOOS == "linux" ]); then
    ...
    if ([ $XGOARCH == "." ] || [ $XGOARCH == "arm-7-el" ]); then
      if [ "$GO_VERSION" -lt 150 ]; then
        echo "Go version too low, skipping linux/arm-7-el..."
      else
        echo "Compiling for linux/arm-7-el..."
        ...
        CC=arm-linux-gnueabi-gcc-6 CXX=arm-linux-gnueabi-g++-6 GOOS=linux GOARCH=arm GOARM=7 CGO_ENABLED=1 CGO_CFLAGS="-march=armv7-a -fPIC" CGO_CXXFLAGS="-march=armv7-a -fPIC" go build $V $X $TP $MOD "${T[@]}" --ldflags="$V $LD" $BM -o "/build/$NAME-linux-arm-7-el`extension linux`" $PACK_RELPATH
      fi
    fi
