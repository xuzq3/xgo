...
for TARGET in $TARGETS; do
  ...
  if ([ $XGOOS == "." ] || [ $XGOOS == "linux" ]); then
    ...
    if ([ $XGOARCH == "." ] || [ $XGOARCH == "arm" ] || [ $XGOARCH == "arm-5" ]); then
    ...
    if ([ $XGOARCH == "." ] || [ $XGOARCH == "arm-7" ]); then
      if [ "$GO_VERSION" -lt 150 ]; then
        ...
        CC=arm-linux-gnueabi-gcc-6 CXX=arm-linux-gnueabi-g++-6 HOST=arm-linux-gnueabi PREFIX=/usr/arm-linux-gnueabi CFLAGS="-march=armv7-a -fPIC" CXXFLAGS="-march=armv7-a -fPIC" $BUILD_DEPS /deps ${DEPS_ARGS[@]}
        export PKG_CONFIG_PATH=/usr/arm-linux-gnueabi/lib/pkgconfig
  
        if [[ "$USEMODULES" == false ]]; then
          CC=arm-linux-gnueabi-gcc-6 CXX=arm-linux-gnueabi-g++-6 GOOS=linux GOARCH=arm GOARM=7 CGO_ENABLED=1 CGO_CFLAGS="-march=armv7-a -fPIC" CGO_CXXFLAGS="-march=armv7-a -fPIC" go get $V $X "${T[@]}" --ldflags="$V $LD" -d $PACK_RELPATH
        fi
        CC=arm-linux-gnueabi-gcc-6 CXX=arm-linux-gnueabi-g++-6 GOOS=linux GOARCH=arm GOARM=7 CGO_ENABLED=1 CGO_CFLAGS="-march=armv7-a -fPIC" CGO_CXXFLAGS="-march=armv7-a -fPIC" go build $V $X $TP $MOD "${T[@]}" --ldflags="$V $LD" $BM -o "/build/$NAME-linux-arm-7`extension linux`" $PACK_RELPATH
      fi
    fi
    if ([ $XGOARCH == "." ] || [ $XGOARCH == "arm-7hf" ]); then
      if [ "$GO_VERSION" -lt 150 ]; then
        echo "Go version too low, skipping linux/arm-7hf..."
      else
        echo "Compiling for linux/arm-7hf..."
        ...
        CC=arm-linux-gnueabihf-gcc-6 CXX=arm-linux-gnueabihf-g++-6 GOOS=linux GOARCH=arm GOARM=7 CGO_ENABLED=1 CGO_CFLAGS="-march=armv7-a -fPIC" CGO_CXXFLAGS="-march=armv7-a -fPIC" go build $V $X $TP $MOD "${T[@]}" --ldflags="$V $LD" $BM -o "/build/$NAME-linux-arm-7hf`extension linux`" $PACK_RELPATH
      fi
    fi
    ...
    if ([ $XGOARCH == "." ] || [ $XGOARCH == "mips64" ]); then
    ...
    if ([ $XGOARCH == "." ] || [ $XGOARCH == "mips64le" ]); then
      if [ "$GO_VERSION" -lt 170 ]; then
    ...
    if ([ $XGOARCH == "." ] || [ $XGOARCH == "ppc64le" ]); then
      if [ "$GO_VERSION" -lt 170 ]; then
    ...
    if ([ $XGOARCH == "." ] || [ $XGOARCH == "mipsle" ]); then
      if [ "$GO_VERSION" -lt 180 ]; then
  ...
  if [ $XGOOS == "." ] || [[ $xgoos == android* ]]; then
    ...
    if [ $XGOARCH == "." ] || [ $XGOARCH == "arm" ] ; then
      ...
      if [[ "$USEMODULES" == false ]]; then
        CC=armv7a-linux-androideabi$PLATFORM-clang CXX=armv7a-linux-androideabi$PLATFORM-clang++ GOOS=android GOARCH=arm CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go get $V $X "${T[@]}" --ldflags="$V $LD" -d ./$PACK_RELPATH
      fi
      CC=armv7a-linux-androideabi$PLATFORM-clang CXX=armv7a-linux-androideabi$PLATFORM-clang++ GOOS=android GOARCH=arm CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go build $V $X "${T[@]}" --ldflags="$V $EXT_LDPIE $EXT_LDAMD $LD" $BM -o "/build/$NAME-android-$PLATFORM-arm`extension android`" ./$PACK_RELPATH
    fi
    if [ $XGOARCH == "." ] || [ $XGOARCH == "arm64" ]; then
      echo "Compiling for android-$PLATFORM/arm64..."
      if [[ "$USEMODULES" == false ]]; then
        CC=aarch64-linux-android$PLATFORM-clang CXX=aarch64-linux-android$PLATFORM-clang++ GOOS=android GOARCH=arm64 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go get $V $X "${T[@]}" --ldflags="$V $LD" -d ./$PACK_RELPATH
      fi
      CC=aarch64-linux-android$PLATFORM-clang CXX=aarch64-linux-android$PLATFORM-clang++ GOOS=android GOARCH=arm64 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go build $V $X "${T[@]}" --ldflags="$V $EXT_LDPIE $EXT_LDAMD $LD" $BM -o "/build/$NAME-android-$PLATFORM-arm64`extension android`" ./$PACK_RELPATH
    fi
    if [ $XGOARCH == "." ] || [ $XGOARCH == "386" ]; then
      echo "Compiling for android-$PLATFORM/386..."
      if [[ "$USEMODULES" == false ]]; then
        CC=i686-linux-android$PLATFORM-clang CXX=i686-linux-android$PLATFORM-clang++ GOOS=android GOARCH=386 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go get $V $X "${T[@]}" --ldflags="$V $LD" -d ./$PACK_RELPATH
      fi
      CC=i686-linux-android$PLATFORM-clang CXX=i686-linux-android$PLATFORM-clang++ GOOS=android GOARCH=386 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go build $V $X "${T[@]}" --ldflags="$V $EXT_LDPIE $EXT_LDAMD $LD" $BM -o "/build/$NAME-android-$PLATFORM-386`extension android`" ./$PACK_RELPATH
    fi
    if [ $XGOARCH == "." ] || [ $XGOARCH == "amd64" ]; then
      echo "Compiling for android-$PLATFORM/amd64..."
      if [[ "$USEMODULES" == false ]]; then
        CC=x86_64-linux-android$PLATFORM-clang CXX=x86_64-linux-android$PLATFORM-clang++ GOOS=android GOARCH=amd64 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go get $V $X "${T[@]}" --ldflags="$V $LD" -d ./$PACK_RELPATH
      fi
      CC=x86_64-linux-android$PLATFORM-clang CXX=x86_64-linux-android$PLATFORM-clang++ GOOS=android GOARCH=amd64 CGO_ENABLED=1 CGO_CFLAGS="$CGO_CCPIE" CGO_CXXFLAGS="$CGO_CCPIE" CGO_LDFLAGS="$CGO_LDPIE" go build $V $X "${T[@]}" --ldflags="$V $EXT_LDPIE $EXT_LDAMD $LD" $BM -o "/build/$NAME-android-$PLATFORM-amd64`extension android`" ./$PACK_RELPATH
    fi
  fi
done
